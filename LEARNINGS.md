...
- Routine production E2E checks reuse the dedicated low-privilege `betstan-e2e` account with the non-secret password `test1234`: log in first and create it only when absent, elevate it only for the bounded administrator action, always revoke it to `USER`, and delete only exact proven acceptance drafts rather than deleting the account.
