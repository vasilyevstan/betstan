...
it('shows the exact partial offer and all original identities before a separate confirmation, without repricing', async () => {
  ...
  expect(axios.post.mock.calls[1][1]).toEqual({ action: 'CONFIRM', clientOperationId: request.clientOperationId, quoteId: `quote-${request.clientOperationId}` });
...
it('replays the saved confirmation after an ambiguous unsubmitted POST, even if its quote has now expired', async () => {
  jest.useFakeTimers();
  jest.setSystemTime(new Date('2026-09-24T12:00:00Z'));
  installApi();
  mount();
