...
# Exact temporary-access identity retirement operator.
...
# recovery workflow. Retains the general betstan-github-sp and AZURE_CREDENTIALS.
...
load_metadata() {
  ...
  # Validate non-empty exact fields
  local field
  ...
  # Reject duplicate object IDs (migration vs recovery must differ)
  ...
  # Role assignment IDs must all differ
...
validate_preconditions() {
  [[ "$RECOVERY_ENABLED" == "false" ]] ||
    die "recovery_must_be_disabled"
  [[ "$ARM_COUNT" == "0" ]] ||
    die "arm_count_must_be_zero"
}
...
verify_retained_identity() {
  # Prove the general betstan-github-sp still exists
  local sp_list
  sp_list="$(az ad sp list --display-name "$RETAINED_SP_DISPLAY_NAME" \
    --query '[].displayName' -o tsv)" ||
    die "retained_sp_query_failed"
  echo "$sp_list" | grep -qxF "$RETAINED_SP_DISPLAY_NAME" ||
    die "retained_sp_not_found:$RETAINED_SP_DISPLAY_NAME"

  # Prove repository AZURE_CREDENTIALS secret exists
  gh secret list --repo "$GH_REPOSITORY" |
    grep -qF "$RETAINED_SECRET_NAME" ||
...
verify_environments() {
  local expected_migration_env expected_recovery_env
  expected_migration_env="$(env_value "$METADATA_FILE" migration_environment)"
  expected_recovery_env="$(env_value "$METADATA_FILE" recovery_environment)"
  [[ "$expected_migration_env" == "$MIGRATION_ENV" ]] ||
    die "wrong_migration_environment"
  [[ "$expected_recovery_env" == "$RECOVERY_ENV" ]] ||
    die "wrong_recovery_environment"
}
...
delete_role_assignment() {
  ...
  if [[ "$rc" -ne 0 ]]; then
    # Tolerate already-absent (check if still present)
...
delete_custom_role() {
  ...
  if [[ "$rc" -ne 0 ]]; then
    local count
    count="$(az role definition list --name "$role_id" --query 'length(@)' -o tsv 2>/dev/null || echo 0)"
    if [[ "$count" != "0" ]]; then
...
verify_role_assignment_absent() {
  local assignment_id="$1"
  local result
  result="$(az role assignment list --query "[?id=='$assignment_id']" -o tsv 2>/dev/null || true)"
...
verify_custom_role_absent() {
  local role_id="$1"
  local count
  count="$(az role definition list --name "$role_id" --query 'length(@)' -o tsv 2>/dev/null || true)"
...
case "$MODE" in
  ...
  verify)
    ...
    # Cleanup metadata only if explicit safe cleanup flag
