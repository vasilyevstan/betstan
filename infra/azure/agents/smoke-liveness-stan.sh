...
IGNORE_ENDPOINT_REGEX="${IGNORE_ENDPOINT_REGEX:-}"
...
if [[ -n "$IGNORE_ENDPOINT_REGEX" ]]; then
  missing_endpoints="$(kubectl get endpoints -n default --no-headers | awk -v ignore_re="$IGNORE_ENDPOINT_REGEX" '$2=="<none>" && $1 !~ ignore_re {print $1}')"
else
