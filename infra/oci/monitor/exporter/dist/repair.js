...
exports.operationDocument = operationDocument;
...
const workflowPath = process.env.REPAIR_WORKFLOW_PATH ??
    ".github/workflows/oci-production-self-heal.yml";
const subject = process.env.REPAIR_OIDC_SUBJECT ??
    `repo:${repository}:environment:oci-production`;
const enabled = process.env.SELF_HEAL_ENABLED === "true";
const allowed = new Set((process.env.SELF_HEAL_SERVICES ?? "client,backoffice")
    .split(",")
    .filter(Boolean));
...
async function kubernetes(path, method, payload) {
    const [token, ca] = await Promise.all([
        (0, promises_1.readFile)("/var/run/secrets/kubernetes.io/serviceaccount/token", "utf8"),
        (0, promises_1.readFile)("/var/run/secrets/kubernetes.io/serviceaccount/ca.crt"),
    ]);
...
function configRecord(configMap) {
...
async function writeOperation(configMap, operation) {
    ...
    return kubernetes(`/api/v1/namespaces/${encodedNamespace}/configmaps/${operationConfigMap}`, "PUT", {
        apiVersion: "v1",
        kind: "ConfigMap",
        metadata: {
...
function textClaim(identity, name) {
...
function runNumber(identity, name) {
...
function operationDocument(previous, identity, repairId, targetSha, phase, now) {
    ...
    return {
        ...
        phase,
        expected_transient_codes: phase === "restarting" ? restartingTransients : [],
        heartbeat_at: iso(now),
        expires_at: iso(new Date(now.getTime() + (phase === "restarting" ? 600 : 900) * 1000)),
        state: "active",
    };
}
function terminalOperation(current, result, now) {
...
async function authenticatedIdentity(request) {
    ...
    return (0, auth_1.verifyGithubOidc)(authorization.slice("Bearer ".length), {
        audience,
        repository,
        workflowPath,
...
function validateCommonRequest(requestBody) {
...
async function waitForRollout(encodedNamespace, service, minimumGeneration) {
    const path = `/apis/apps/v1/namespaces/${encodedNamespace}/deployments/` +
        `gaming-${service}-depl`;
...
async function restart(request, identity) {
    ...
    const encodedNamespace = encodeURIComponent(namespace);
    const operationPath = `/api/v1/namespaces/${encodedNamespace}/configmaps/${operationConfigMap}`;
    const [releaseMap, currentOperationMap, deployment] = await Promise.all([
        kubernetes(`/api/v1/namespaces/${encodedNamespace}/configmaps/${releaseConfigMap}`, "GET"),
        kubernetes(operationPath, "GET"),
        kubernetes(`/apis/apps/v1/namespaces/${encodedNamespace}/deployments/gaming-${service}-depl`, "GET"),
    ]);
    ...
    if (release.schema !== "betstan.active-release.v1" ||
    ...
    const now = new Date();
    const activeOperation = operationDocument(currentOperation, identity, repairId, targetSha, "restarting", now);
    let ownedOperationMap = await writeOperation(currentOperationMap, activeOperation);
    try {
        const restartedAt = iso(new Date());
        const patched = await kubernetes(`/apis/apps/v1/namespaces/${encodedNamespace}/deployments/gaming-${service}-depl`, "PATCH", {
            spec: {
                template: {
                    metadata: {
                        annotations: {
        ...
        try {
            ...
            if (current.operation_id === activeOperation.operation_id &&
                current.repair_id === repairId &&
                current.state === "active") {
                await writeOperation(currentMap, terminalOperation(current, "failed", new Date()));
            }
        }
        catch {
...
async function finalize(request, identity) {
    const requestBody = await body(request);
    ...
    const runId = runNumber(identity, "run_id");
    const runAttempt = runNumber(identity, "run_attempt");
    if (operation.operation_id !== `self-heal-${runId}-${runAttempt}` ||
...
async function handler(request, response) {
    ...
    try {
        ...
        const result = request.url === "/__betstan/repair/v1/restart"
        ...
        const unauthorized = reason.includes("OIDC") ||
