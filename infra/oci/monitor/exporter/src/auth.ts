...
let cachedJwks: { expiresAt: number; value: JwkSet } | undefined;
...
function textClaim(payload: JsonObject, name: string): string {
...
function numericClaim(payload: JsonObject, name: string): number {
  const value = payload[name];
...
async function defaultJwksFetcher(): Promise<JwkSet> {
...
export async function verifyGithubOidc(
  token: string,
  policy: OidcPolicy,
  jwksFetcher: () => Promise<JwkSet> = defaultJwksFetcher,
): Promise<JsonObject> {
  ...
  const expectedWorkflowRef =
    `${policy.repository}/${policy.workflowPath}@${policy.ref}`;
