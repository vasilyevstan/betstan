    ...
    dist/scripts/rescheduleSyntheticEvent.js:apply)
      args='
          args:
            ...
            - "apply"
            - "--confirmation"
            - "RESCHEDULE_EVENT:42643b4c173d1c7b8eeed765:2026-09-11T08:05:00.000Z"'
      ;;
...
project_reschedule_report() {
  ...
  jq -e \
    ...
    --arg stage "$stage" \
    --arg expected_mode "$expected_mode" \
    --arg target_event_id "42643b4c173d1c7b8eeed765" \
    --arg target_kickoff "2026-09-11T08:05:00.000Z" '
      def nonnegative_integer:
        type == "number" and . >= 0 and . == floor;
