...
grep -Fq 'type: LoadBalancer' "$OCI_DIR/helm/ingress-nginx-values.yaml" ||
  oci_die "ingress-nginx must create the only LoadBalancer service"
grep -Fq 'shape-flex-min: "10"' "$OCI_DIR/helm/ingress-nginx-values.yaml" ||
  oci_die "ingress minimum bandwidth is not fixed at 10 Mbps"
grep -Fq 'shape-flex-max: "10"' "$OCI_DIR/helm/ingress-nginx-values.yaml" ||
  oci_die "ingress maximum bandwidth is not fixed at 10 Mbps"
...
oci_require_vars \
  ...
  OCI_COMPARTMENT_OCID OCI_COMPARTMENT_NAME OCI_AVAILABILITY_DOMAIN \
  OCI_KUBERNETES_VERSION OCI_NODE_IMAGE_OCID OCI_NODE_POOL_NAME
oci_require_vars \
...
oci_require_ocid OCI_NODE_IMAGE_OCID
...
[[ "$existing_pool_count" -le 1 ]] || oci_die "multiple managed node pools share OCI_NODE_POOL_NAME"
...
image_shapes="$(oci compute image-shape-compatibility-entry list --image-id "$OCI_NODE_IMAGE_OCID" --all)"
...
jq -e '[.data[]? | select(.shape == "VM.Standard.A1.Flex")] | length == 1' \
  ...
  oci_die "OCI_NODE_IMAGE_OCID is not compatible with VM.Standard.A1.Flex"
